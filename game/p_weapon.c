...
void Spear_Fire(edict_t* ent, vec3_t g_offset, int damage, qboolean hyper, int effect)
{
	...
	fire_spear(ent, start, forward, damage, 1000, 120, 120);
...
void Sword_Fire(edict_t* ent, vec3_t g_offset, int damage, qboolean hyper, int effect)
{
	...
	fire_sword(ent, start, forward, damage, 650, 120, 120);
...
void Weapon_Sword_Fire(edict_t* ent)
{
	...
	if (deathmatch->value)
		...
		damage = 10;
	Sword_Fire(ent, vec3_origin, damage, false, EF_BLASTER);
	ent->client->ps.gunframe++;
}
...
void Gauntlet_Fire(edict_t* ent, vec3_t g_offset, int damage, qboolean hyper, int effect)
{
	...
	fire_gauntlet(ent, start, forward, damage, 650, 120, 120);
...
void Weapon_Gauntlet(edict_t* ent)
{
	static int	pause_frames[] = { 19, 32, 0 };
	static int	fire_frames[] = { 5, 0 };

	Weapon_Generic(ent, 4, 8, 52, 55, pause_frames, fire_frames, Weapon_Gauntlet_Fire);
}
